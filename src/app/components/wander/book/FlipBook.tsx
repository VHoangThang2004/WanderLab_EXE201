...
const Page = forwardRef<HTMLDivElement, PageProps>((props, ref) => {
  ...
  return (
    <div className="page bg-[#fdfaf6] border-r border-[#e0d6c8] shadow-[inset_-20px_0_40px_rgba(0,0,0,0.05)] p-8 overflow-hidden relative text-slate-800" ref={ref}>
      ...
      <div className="page-footer absolute bottom-4 left-0 w-full text-center text-slate-400 text-sm font-serif">
        {props.number}
      </div>
    </div>
  );
  ...
  const pages = useMemo(() => {
      ...
      <Page key="cover" number={pageNum++} isCover>
        <div className="flex-1 flex flex-col justify-center items-center text-center p-8">
          ...
          <p className="text-slate-400 text-lg uppercase tracking-widest mt-4">
            Tác giả: {user?.user_metadata?.full_name || 'Kẻ Lữ Hành'}
          </p>
    ...
    p.push(
      <Page key="toc" number={pageNum++}>
        <div className="pt-8">
          <h2 className="text-3xl font-serif font-bold text-slate-800 border-b-2 border-orange-200 pb-4 mb-8">Mục Lục</h2>
      ...
      p.push(
        <Page key={`diary-${diary.id}-p1`} number={pageNum++}>
          <div className="h-full flex flex-col pt-4">
            <div className="text-orange-600 font-serif font-bold tracking-widest uppercase text-sm mb-2">Chương {idx + 1}</div>
            <h2 className="text-3xl font-serif font-bold text-slate-900 leading-tight mb-6">{diary.title}</h2>
            ...
            <div className="text-slate-700 leading-relaxed text-justify line-clamp-[8] font-serif">
              {diary.description}
            </div>
          </div>
        </Page>
          ...
          p.push(
            <Page key={`diary-${diary.id}-day-${day.day}`} number={pageNum++}>
               <div className="pt-8 h-full flex flex-col">
                <h3 className="text-xl font-serif font-bold text-slate-800 border-b border-slate-200 pb-2 mb-6">
                  Lịch trình {diary.timeline.length > 1 ? `- Trang ${dayIdx + 1}` : 'chi tiết'}
                </h3>
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  <div className="font-bold text-orange-600 mb-4 font-serif text-lg">Ngày {day.day}: {day.title}</div>
      ...
      if (diary.budgetBreakdown && diary.budgetBreakdown.length > 0) {
        p.push(
          <Page key={`diary-${diary.id}-budget`} number={pageNum++}>
             <div className="pt-8 h-full flex flex-col">
              <h3 className="text-xl font-serif font-bold text-slate-800 border-b border-slate-200 pb-2 mb-6">Chi Phí Chi Tiết</h3>
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        ...
        p.push(
          <Page key={`diary-${diary.id}-p2`} number={pageNum++}>
             <div className="h-full flex items-center justify-center p-12 text-center">
               <div className="text-2xl font-serif italic text-slate-400">
    ...
    p.push(
      <Page key="back-cover" number={pageNum++} isCover>
        <div className="h-full flex flex-col justify-center items-center p-8 bg-slate-900 rounded-l-lg">
          ...
          <h2 className="text-2xl font-serif font-bold text-slate-500 mb-2">WanderLab</h2>
